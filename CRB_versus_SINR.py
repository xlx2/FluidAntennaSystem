...
objval_fas = []
objval_mimo = []
sinr_range = range(12, 17, 1)
for SINR in sinr_range:
    t_start = time.time()
    print(f'Current SINR: {SINR}(dB)')
    fas_parameters['Gamma'] = dB2pow(SINR)
    mimo_parameters['Gamma'] = dB2pow(SINR)
    _, crb_fas = Fluid_Antenna_System(**fas_parameters).run(verbose=False)
    objval_fas.append(crb_fas)
    _, crb_mimo = MIMO_System(**mimo_parameters).run(verbose=False)
...
# 绘图
plt.figure()
plt.plot(sinr_range, objval_fas, 'b-s', linewidth=1.5, label='Proposed')
plt.plot(sinr_range, objval_mimo, 'r-o', linewidth=1.5, label='Traditional')
plt.grid(True)
